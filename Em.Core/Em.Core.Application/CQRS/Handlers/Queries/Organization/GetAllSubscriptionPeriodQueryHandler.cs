...
namespace Em.Core.Application.CQRS.Handlers.Queries.Organization
{
    public class GetAllSubscriptionPeriodQueryHandler : IRequestHandler<GetAllSubscriptionPeriodQuery, IReadOnlyList<GetAllSubscriptionPeriodDto>>
    {
        ...
        public GetAllSubscriptionPeriodQueryHandler(IUnitOfWork unitOfWork)
        {
        ...
        {
            var entities = await _unitOfWork.SubscriptionPeriodRepository.GetAllAsync(cancellationToken);
